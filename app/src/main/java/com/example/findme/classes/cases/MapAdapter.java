...
public class MapAdapter extends RecyclerView.Adapter<MapAdapter.MapViewHolder> {
    ...
    private MapAdapter.OnMapClickListener onMapClickListener;

    public MapAdapter(List<StorageReference> items, Context context) {
        this.items = items;
        this.context = context;
        this.onMapClickListener = (MapAdapter.OnMapClickListener) context;
    }
    ...
    public void onBindViewHolder(@NonNull MapViewHolder holder, int position) {
        ...
        item.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
            ...
            public void onSuccess(Uri uri) {
                if(item.getName().endsWith(".link")) {
                    holder.ivImage.setImageResource(R.drawable.img_not_entry);
                    holder.itemView.findViewById(R.id.cvItem).setOnClickListener(v -> onMapClickListener.onMapClick(uri, true));
                } else {
                    Glide.with(context).load(uri).into(holder.ivImage);
                    holder.itemView.findViewById(R.id.cvItem).setOnClickListener(v -> onMapClickListener.onMapClick(uri, false));
                }
            }
        });
    ...
    public interface OnMapClickListener {
        void onMapClick(Uri uri, boolean isLink);
    }
}
